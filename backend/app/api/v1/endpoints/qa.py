...
from app.services.conversation_manager import ChatSessionService
...
async def _generate_and_store_embeddings(
    ...
    """
    Fallback: generate and store embeddings for clauses when they are missing.
    Called only when a document is marked COMPLETED but embeddings were never stored.
    """
    texts: List[str] = []
    clause_ids: List[str] = []

    for clause in clauses:
        text = clause.get("summary") or clause.get("original_text", "")
        if text.strip():
            texts.append(text)
            clause_ids.append(clause.get("clause_id", ""))

    if not texts:
        raise EmbeddingsError("No valid text found in clauses for fallback embedding generation")
    ...
    if embeddings_data:
        ...
        logger.info(f"Fallback embeddings stored: {len(embeddings_data)} clauses for document {doc_id}")
    else:
        raise EmbeddingsError("Fallback embeddings generation produced no valid vectors")
