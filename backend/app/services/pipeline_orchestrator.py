...
from app.services.docai_processor import DocumentProcessor as DocumentProcessorHTTP
from app.services.contract_segmenter import ClauseSegmenter, ClauseCandidate
from app.services.llm_client import GeminiClient, GeminiError
from app.services.firestore_client import FirestoreClient, FirestoreError
from app.services.privacy_service import PrivacyService
from app.services.risk_engine import RiskAnalyzer, RiskAssessment
from app.services.text_clarity import ReadabilityService
from app.services.vector_search import EmbeddingsService, EmbeddingsError
    ...
    async def process_document_complete(
        ...
        with LogContext(logger, doc_id=doc_id, filename=filename, session_id=session_id):
            ...
            try:
                ...
                # Stage 7.5: Generate Embeddings for Clauses
                logger.info("Stage 7.5: Generating embeddings for semantic search")
                embeddings_count = 0
                try:
                    embeddings_count = await self._generate_clause_embeddings(doc_id, clauses_data)
                    processing_result["stages_completed"].append("embeddings_generated")
                    logger.info(f"Generated {embeddings_count} embeddings for {len(clauses_data)} clauses")
                except Exception as emb_error:
                    logger.error(f"Embeddings generation failed (non-fatal): {emb_error}")
                    processing_result["stages_completed"].append("embeddings_failed")
                ...
                final_metadata = {
                    ...
                    "processing_statistics": {
                        ...
                        "embeddings_completed": "embeddings_generated" in processing_result["stages_completed"],
