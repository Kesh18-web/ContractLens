...
from app.services.contract_segmenter import ClauseCandidate
...
• IMPORTANT: Keep legal terms in English but provide explanations in the target language"""
    ...
    async def _generate_text(self, system_prompt: str, user_prompt: str) -> str:
        """Generate plain-text content (not JSON) using the GenAI client."""
        if not self._client:
            raise GeminiError("Client not initialized")
    ...
    async def summarize_conversation(
        ...
        """
        Summarize a chat conversation history into a short paragraph.

        Args:
            conversation_text: Formatted conversation transcript (role: content per line).
            max_words: Target word limit for the summary.
        ...
        system_prompt = (
            ...
            "Your output must be plain text — no JSON, no bullet points, no markdown. "
            "Focus on: main topics discussed, key questions asked, important conclusions, "
            "and which documents were referenced."
        )
        user_prompt = (
            f"Summarize the following conversation in under {max_words} words:\n\n"
            f"{conversation_text}"
        )

        try:
            summary = await self._generate_text(system_prompt, user_prompt)
